...
class RegionServer113(val clusterStatus: ClusterStatus, val serverNameObj: ServerName, override val infoPort: Int) extends RegionServer {
  ...
  override lazy val regionsLoad = load.getRegionsLoad().values.asScala.map( new RegionLoad113(_) )

  override def equals(that: Any) = {
    that match {
      case other: RegionServer113 => other.serverNameObj == serverNameObj
      case _ => false
    }
  }
}
